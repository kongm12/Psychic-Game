  var wins = 0;
    ...
    document.onkeyup = function(event) {
        ...
        if (userGuess == computerGuess) {
            wins++;
            alert("You've guessed corrrectly. You Won!");
            guessesLeft = 10; //reseting the guesses back to 9 so that the user can play again
            guessesSoFar.length = 0; // removes everything from the guesses so far array, so that the guesses from the previous round don't show
        }
